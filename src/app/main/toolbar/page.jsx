...
const Toolbar = ({ executeCommand, fontSize, handleFontSizeChange, fontSizes, editor, handleAddParagraph }) => {
    ...
    return (
        <div className="ml-4 sm:ml-6 md:ml-12 lg:ml-24">
            <div className={`flex items-center border-b rounded-full bg-purple-300 p-2 transition-all duration-500 ease-in-out`}
                style={{
                    width: isExpanded ? "100%" : "3rem",
                    maxWidth: isExpanded ? "32rem" : "3rem",
                    overflow: "hidden",
                    height: "3rem",
                }}
            >
                ...
                <div className={`flex flex-wrap items-center space-x-2 ml-2 transition-opacity duration-500 ease-in-out`}
                    style={{
                ...
                >
                    <select
                        value={fontSize}
                        onChange={handleFontSizeChange}
                        className="px-2 py-1 border rounded-md text-sm sm:px-4 sm:py-2 bg-white text-black"
                    >
                        {fontSizes?.map((size) => (
                            <option key={size} value={size}>
                                {size}px
                    ...
                    <button onClick={() => executeCommand("bold")} className="p-2 hover:bg-gray-200 rounded">
                        <FaBold className="text-lg" />
                    </button>
                    <button onClick={() => executeCommand("italic")} className="p-2 hover:bg-gray-200 rounded">
                        <FaItalic className="text-lg" />
                    </button>
                    <button onClick={() => executeCommand("underline")} className="p-2 hover:bg-gray-200 rounded">
                        <FaUnderline className="text-lg" />
                    </button>
                    <button onClick={handleAddParagraph} className="p-2 hover:bg-gray-200 rounded">
                        <FaParagraph className="text-lg" />
                    </button>
                </div>
            </div>
